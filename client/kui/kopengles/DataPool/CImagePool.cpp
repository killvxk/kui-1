...
namespace OpenHMI
{
	...
	CCacheImage*  CImagePool::getImage(const String &strFilePath)
	{
		...
		pCacheImage->setName(strFilePath);

		return pCacheImage;
	}

	CCacheImage*  CImagePool::getImage(const String &strFilePath, Gdiplus::Bitmap* pBitmap)
	{
		CCacheImage *pCacheImage = (CCacheImage*)(CacheObject*)m_cachePool.getCacheObject(strFilePath);
		if (pCacheImage != NULL)
		{
	...
	void CImagePool::freeImage(CCacheImage* pImage)
	{
		;
	}
}