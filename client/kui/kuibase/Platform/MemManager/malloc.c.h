...
#define DEFAULT_TRIM_THRESHOLD ((size_t)40U * (size_t)1024U * (size_t)1024U)
...
static FORCEINLINE int pthread_acquire_lock (MLOCK_T *sl) {
  ...
  for (;;) {
    if (*lp != 0) {
      ...
      int ret;
      #if defined(__QNX__)
		ret = _smp_cmpxchg(lp, cmp, val) == 0 ? 1 : 0;
      #else
	...
	thr_yield();
#else
#if defined(__linux__) || defined(__FreeBSD__) || defined(__APPLE__) || defined(__QNX__)
	sched_yield();
