...
BOOL CPing::Ping(char *szDestIP,char* ip, PingReply *pPingReply, DWORD dwTimeout)
{
	struct hostent *ptr = gethostbyname(szDestIP);
	if (NULL == ptr){
		return FALSE;
	}
	in_addr * iddr = (in_addr*)ptr->h_addr;
	strcpy_s(ip,64, inet_ntoa(*iddr));
	if (NULL != ip)
	{
		return PingCore(inet_addr(ip), pPingReply, dwTimeout);
	}
	return FALSE;
}
