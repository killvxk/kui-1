...
CZipAd* CZipAd::getInstance()
{
	static CZipAd skin;
	return &skin;
}

bool CZipAd::LoadZip(const CString& strPath)
{
	m_hZip = NULL;
	m_pZipFile = NULL;
...
bool CZipAd::FindFileFromZip(LPCTSTR lpFile)
{
...
Gdiplus::Bitmap* CZipAd::LoadImageFromZip(LPCTSTR lpFile)
{
...
bool CZipAd::LoadFileFromZipEx(LPCTSTR lpFile, unsigned char** buffer, unsigned int* size)
{
	if (lpFile == NULL || m_hZip == NULL) return false;
