...
const unsigned char vnc_key[] = { 23, 82, 107, 6, 35, 78, 88, 7 };

bool AfxEncryptVnc(char* plain, int len,unsigned char* crypted)
{
	DesCrypt		des;
	unsigned char	*pass = new unsigned char[len];
	memset(pass,len,0);

	for(size_t i = 0; i < strlen(plain); i++)
		pass[i] = plain[i];

	des.encrypt(crypted, pass, len, vnc_key);
...
bool AfxDecryptVnc(unsigned char* crypted, char* plain, int len)
{
	...
	des.decrypt((unsigned char*)plain, crypted, len, vnc_key);
...
{
public:
	TEA(const TCHAR *key, int round = 32, bool isNetByte = false);
	TEA(const TEA &rhs);
	TEA& operator=(const TEA &rhs);
	void encrypt(const TCHAR *in, TCHAR *out);
	void decrypt(const TCHAR *in, TCHAR *out);
private:
...
private:
	...
	TCHAR _key[16]; //encrypt or decrypt key
};

TEA::TEA(const TCHAR *key, int round /*= 32*/, bool isNetByte /*= false*/):_round(round) ,_isNetByte(isNetByte) 
{
	memset(_key, 0, sizeof(_key));
	if (key != 0)
		memcpy(_key, key, sizeof(TCHAR)*_tcslen(key));
}

TEA::TEA(const TEA &rhs):_round(rhs._round),_isNetByte(rhs._isNetByte)
{
	memset(_key, 0, sizeof(_key));
	memcpy(_key, rhs._key, sizeof(TCHAR)*_tcslen(rhs._key));
}
...
void TEA::encrypt(const TCHAR *in, TCHAR *out) 
{
	encrypt((const ulong*)in, (ulong*)out);
}

void TEA::decrypt(const TCHAR *in, TCHAR *out) 
{
...
{
	int nLenth = 0;
	TEA tea(pkey, round, isNetByte);
	TCHAR* pTempCrypt = pCrypt;
	for (int i=0;i< nlainlength;i+=4)
	{
		TCHAR crypt[5] = {0};
		tea.encrypt(plain+i, crypt);
		memcpy(pTempCrypt,crypt,sizeof(TCHAR)*4);
		nLenth += 4;
		pTempCrypt += 4;
	}
	nCryptLength = nLenth;
...
{
	TEA tea(pkey, round, isNetByte);
	TCHAR* pTemplain = plain;
	for (int i=0;i< nCryptLength;i+=4)
	{
		TCHAR lain[5] = {0};
		tea.decrypt(pCrypt+i, lain);
		memcpy(pTemplain,lain,sizeof(TCHAR)*4);
		pTemplain += 4;
	}
}
