...
namespace serialize { namespace detail {
	...
	struct select_traits_t< std::chrono::duration<T, U> >
	{
		static const bool is_defined_type = true;

		template < typename StreamT >
		static void push(StreamT &os, const std::chrono::duration<T, U> &val)
		{
			os << val.count();
		}

		template < typename StreamT >
		static void pop(StreamT &os, std::chrono::duration<T, U> &val)
		{
			T v = 0;
			os >> v;

			val = std::chrono::duration<T, U>{v};
		}
	};
